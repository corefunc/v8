 ...
 * @returns {boolean}
 */
function checkIsPrimitive(value: unknown): boolean {
 ...
 */
function objectKeysSort<ObjectType extends Record<string, any>>(
 ...
 * @returns {string}
 */
function hashToVersion(hash: string, version: string = "5"): string {
  return (
    hash.substring(0, 8) +
    "-" +
    hash.substring(8, 12) +
